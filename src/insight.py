...
503/429 등 일시적 오류에 대비한 강화된 폴백 정책:
  1) gemini-2.5-flash 최대 3회 (5초, 15초, 30초 대기)
  2) gemini-2.5-flash-lite 최대 2회 (5초, 15초 대기)
  3) gemini-2.0-flash 최대 2회 (5초, 15초 대기)
  4) gemini-2.0-flash-lite 최대 2회 (5초, 15초 대기)

총 최악의 경우 약 200초 대기 후 실패 처리되지만,
보통은 첫 1-2회 시도 또는 폴백 1단계 안에서 성공합니다.
"""
...
# 폴백 순서: (모델명, 최대 시도 횟수)
# 트래픽 피크 시간에 한 모델군이 마비돼도 다른 세대 모델로 살아남도록 다양화.
MODEL_FALLBACK = [
    (config.GEMINI_MODEL, 3),         # gemini-2.5-flash (메인)
    ("gemini-2.5-flash-lite", 2),     # 같은 세대, 가벼운 모델
    ("gemini-2.0-flash", 2),          # 이전 세대, 트래픽 적음
    ("gemini-2.0-flash-lite", 2),     # 최후 폴백
]

# 재시도 대기 시간 (초). 시도 횟수에 따라 인덱스 사용.
# 503은 보통 5-30초 사이 회복되므로 30초까지 기다림.
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                if retryable and not is_last_attempt:
                    # attempt 1 → RETRY_DELAYS[0] = 5초
                    # attempt 2 → RETRY_DELAYS[1] = 15초
